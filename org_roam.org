...
** Skipping over buffers
There are like to be many buffers open for org-roam nodes, and so when
navigating across buffers for cleaning things up I'd prefer skipping
those.

Tweak [[id:B11A5ECF-B961-4722-885E-B9965938E615][switch-to-prev-buffer-skip]] to add [[id:58A82522-0B48-4714-BE1D-6B53393C716E][org-roam-node-buffer-p]].
Also skip any bufffers already in windows.

This should likely be rewritten to be a hook or similar so it can be
decomposed.

*** TODO It looks as though this doesn't get along with xwidgets

Wrap in a [[id:0E00106D-1AED-4498-8A15-2511B9B7E830][condition-case]] to surface information when some buffer or
other causes issues.
    ...
    (lambda (window buffer bury-or-kill)
  	...
  	(save-excursion
  	  (or (get-buffer-window buffer t)
                (cabinet-buffer-p buffer)))
        (t (message "%s" buffer)))))
#+end_src

#+RESULTS:
: #[(window buffer bury-or-kill) ((save-excursion (or (get-buffer-window buffer t) (cabinet-buffer-p buffer)))) nil]
...
** cabinet-buffer-p
:PROPERTIES:
:ID:       58A82522-0B48-4714-BE1D-6B53393C716E
:END:

Provide a predicate to determine whether a given buffer is a cabinet
file.

Originally this was written to try to use richer introspection...but
checking mode and directory seems simpler and sufficient.

The cabinet directory should likely be updates to just leverage the
org directory also.

#+begin_src emacs-lisp
  (defun cabinet-buffer-p (buffer)
