...
#+begin_src emacs-lisp :results silent
  (write-region (describe-variable 'org-directory) nil "org-directory.out")
#+end_src
#+transclude: [[./org-directory.out]]
#+begin_src emacs-lisp :results silent
...
#+begin_src emacs-lisp
  (describe-variable 'org-link-frame-setup)
#+end_src

#+RESULTS:
#+begin_example
org-link-frame-setup is a variable defined in ‘ol.el’.
...
#+begin_src emacs-lisp :results output
  (setq org-publish-project-alist
        (list (list "cabinet"
  		  :base-directory (format "%s/cabinet" org-directory)
  		  :publishing-function #'org-html-publish-to-html
  		  :publishing-directory (format "%s/pub" org-directory))))
#+end_src
...
Set this based on files collected from [[id:141D8A2B-2353-4636-9E02-3C9899EB1174][org-directory]].

#+begin_src emacs-lisp :results output
  (defun mw/agenda-files ()
    (setq org-agenda-files
  	(directory-files-recursively
  	 org-directory
  	 "\\.org$"
  	 nil
  	 (lambda (dir) (unless (string-match-p "/vendor/" dir) 1)))))
  (mw/agenda-files)
#+end_src
...
#+begin_src emacs-lisp
  (describe-function #'org-agenda-open-link)
#+end_src

#+RESULTS:
: org-agenda-open-link is an interactive byte-code-function in
: ‘org-agenda.el’.
: 
...
This function doesn't seem to be adheering to display-window-alist,
but it's probably just a result of my not using it properly...adding
an implementation for debugging....

#+begin_src emacs-lisp
  (defun mw/org-agenda-open-link (&optional arg)
    "Open the link(s) in the current entry, if any.
  This looks for a link in the displayed line in the agenda.
  It also looks at the text of the entry itself."
    (interactive "P")
...
The types themselves aren't readily surfaced so I'll start with
capturing them as needed until I potentially find a decent resource.
*** Types
**** babel-call
:PROPERTIES:
:ID:       0C992EE2-DDF8-4475-B157-C017AEF4F6A1
:END:

A ~call:~ line. This can be passed to [[id:D7816316-280D-4B7B-9420-3E33264E24D2][org-babel-execute-src-block]].

*** Functions
**** org-element-type-p
:PROPERTIES:
:ID:       0593263E-0F14-4414-8AF4-49F164CC3EEC
:END:

#+call: obi/describe-function(function="org-element-type-p")

#+RESULTS:
#+begin_example
org-element-type-p is a byte-code-function in ‘org-element-ast.el’.
...
TYPES can be a type symbol or a list of symbols.
  ...
  This function has a compiler macro ‘org-element-type-p--inliner’.
