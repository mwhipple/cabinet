...
* Style
There are some established style guides I should probably adopt (I
just saw batsov's), but for now I'll just capture some explanations
here.

** [[id:824EE294-DF57-45EA-861F-CCE772A8B83F][types]]

With emacs-lisp I'll gravitate towards the use of gradual typing where
appropriate. There seem to be a fair amount of fairly complicated
compound structures floating around, and I think due to history and
language design they tend to be modeled in very basic forms which just
leads to a repeated need to track down what the different things mean.

For the most part I'll likely look at working with the data as it is
and expand documentation and defined functions, probably leaning
heavily on [[id:E6A8C472-96C7-4B2C-B127-999D94702E5F][cl-deftype]]. I'll likely also add cl-defstruct compatible
functions so that that route is more readily available where/when it
makes sense.

The convention that I'll adopt is to use PascalCase for type names
which matches a convention I think I picked up somewhere working with
Racket...but I may have made it up.
...
*** describe-function
:PROPERTIES:
:ID:       A365ACE6-A6A4-4E8D-97C5-C7D44014A187
:END:

#+call: obi/describe-function(function="describe-function")
...
#+call: obi/describe-function(function="describe-key")

#+RESULTS:
#+begin_example
describe-key is an interactive byte-code-function in ‘help.el’.
...
#+call: obi/describe-function(function="describe-mode")
...
It is bound to h, ?, C-h m, <f1> m and <help> m.
It can also be invoked from the menu: Help → Describe → Describe
Buffer Modes.
...
#+begin_src emacs-lisp
  (defun mw/shortdoc (group)
    "Return shortdoc contents while containing state changes."
    (save-excursion
      (with-temp-buffer
...
*** intern-soft
:PROPERTIES:
:ID:       F60C9D6B-8572-4963-B50C-D612DB8844DE
:END:

#+call: obi/describe-function(function="intern-soft")

#+RESULTS:
#+begin_example
intern-soft is a primitive-function in ‘src/lread.c’.

(intern-soft NAME &optional OBARRAY)

Declared type:
  (function ((or string symbol) &optional (or obarray vector)) symbol)
...
#+RESULTS:
#+begin_example
eq is a primitive-function in ‘src/data.c’.

(eq OBJ1 OBJ2)

Declared type: (function (t t) boolean)

Return t if the two args are the same Lisp object.

  Other relevant functions are documented in the comparison and symbol
  groups.
  Probably introduced at or before Emacs version 1.6.
  This function does not change global state, including the match
    data.
  This function has a compiler macro ‘bytecomp--check-eq-args’ and a
    ‘byte-compile’ property ‘byte-compile-two-args’ and a byte-code
    optimizer ‘byte-optimize-eq’.  See the manual for details.
...
#+begin_src emacs-lisp
  (describe-function #'alist-get)
#+end_src

#+RESULTS:
#+begin_example
alist-get is a byte-code-function in ‘subr.el’.
...
Example:
  ...
  ‘alist-get’ is also a generalized variable.
...
#+RESULTS:
#+begin_example
directory-files-recursively is a byte-code-function in ‘files.el’.
...
#+begin_src emacs-lisp
  (describe-function #'string-join)
#+end_src

#+RESULTS:
#+begin_example
string-join is a byte-code-function in ‘subr-x.el’.
...
* elisp-urls
:PROPERTIES:
:ID:       F1DEB8D0-02EE-4856-9C59-A2D6B51D698D
:END:
** url-encode-url
:PROPERTIES:
:ID:       2140D11D-1066-4B77-9062-0B8A1C5A287D
:END:

#+begin_src emacs-lisp
  (describe-function #'url-encode-url)
#+end_src

#+RESULTS:
#+begin_example
url-encode-url is an autoloaded byte-code-function in ‘url-util.el’.
...
Define some generic [[id:083CCE11-4732-459E-81FE-3738C8920016][dictionary]] behavior.
This could potentially overlap with an existing library like ~a~, but
that will be chased down later.

** For alists and plists

The core types used for dictionaries in elisp are alists and
plists...both of which are just lists. This /could/ simply be left as
generalized and rely on logic that resolves keys in whatever way
works, but it seems better to provide a bit more control both for the
sake of optimization and to allow for preventing unexpected behavior.

This will therefore make use of a struct which wraps the object with a
corresponding implementation. Unless otherwise indicated this is
likely to make use of intentional aliasing/mutability so the struct
can act as a dictionary handle to a structure which may have new or
...
Lookup a value by a key. This will only return the value itself
(rather than something like a cons cell). This will return nil if no
key is found.

#+begin_src emacs-lisp
  (cl-defgeneric mw/dict/get (ctx dict key)
    "Return the value associated with key within dict.")
#+end_src
...
Define the support for generic lists using a wrapper.

To start, ~impl~ will be an alist where the cdrs are functions.
The general signature of the functions will be of the form:
~dict key [value]~.

#+begin_src emacs-lisp
  (cl-defstruct mw/dict/list
    "Wrap a list with defined dictionary behavior."
    impl data)
#+end_src
...
Define the method which leverages the impl.

#+begin_src emacs-lisp
  (cl-defmethod mw/dict/get (ctx (l mw/dict/list) key)
    (funcall (alist-get :get (mw/dict/list-impl l)) (mw/dict/list-data l) key))
#+end_src
...
#+begin_src emacs-lisp
  (defconst mw/dict/list/alist-impl
    (list (cons :get (lambda (dict key) (alist-get key dict)))))
#+end_src

#+RESULTS:
: mw/dict/list/alist-impl
...
#+RESULTS:
: put is a primitive-function in ‘C source code’.
: 
...
** cl-defstruct
:PROPERTIES:
:ID:       BE779DCE-0F34-4DAC-90A9-0D1C16EFBC75
:END:
...
#+begin_src emacs-lisp
  (describe-function #'cl-check-type)
#+end_src
...
** cl-deftype
:PROPERTIES:
:ID:       E6A8C472-96C7-4B2C-B127-999D94702E5F
:END:

#+begin_src emacs-lisp
  (describe-function #'cl-deftype)
#+end_src
